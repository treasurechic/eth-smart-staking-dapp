...
describe("EtherWallet", function () {
  ...
  describe("walletDeposit", function () {
    it("Should deposit Ether to the Wallet contract", async function () {
      ...
      expect(balance.toString()).to.equal(amountToDeposit);
    });
  });
  ...
  describe("stakeEth", function () {
    it("Should stake Ether to the staking contract", async function () {
      ...
      // Stake ETH
      const tx = await staking.stakeEth(0, amountToStake);
      tx.wait();
      ...
      expect(wallet.stakedAmount.toString()).to.equal(amountToStake);
      // eslint-disable-next-line no-unused-expressions
  ...
  describe("currentStake", function () {
    it("Should return the current stake", async function () {
      ...
      const amountToStake = ethers.utils.parseEther("1");
      // Stake ETH
      const txStake = await staking.stakeEth(0, amountToStake);
      txStake.wait();
      ...
      expect(balance.toString()).to.equal(amountToStake);
    });
  });
  ...
  describe("currentReward", function () {
    it("Should get the current staking reward for a wallet", async function () {
      ...
      // Stake ETH
      const txStake = await staking.stakeEth(0, ethers.utils.parseEther("1"));
      txStake.wait();
